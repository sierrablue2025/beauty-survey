...
export function ProductCard({ item, reason }: ProductCardProps) {
  ...
  return (
    ...
    >
      {reason && (
        <div className="bg-[#FAF0EC] px-3 py-1.5 text-center">
          <span className="text-[12px] text-[#C89A82] font-semibold">「{reason}」</span>
        </div>
      )}
      {imageUrl && (
        <div className="w-full aspect-square bg-[#FAF8F6] overflow-hidden relative">
          ...
          {item.reviewCount >= 100 && (
            <span className="bg-[#D8B4A0] text-white text-[11px] font-bold px-2 py-0.5 rounded-br-lg absolute top-0 left-0">
              売れてる
            </span>
          )}
        </div>
      ...
      <div className="p-3 flex flex-col gap-1 flex-1">
        ...
        <p className="text-[15px] font-medium text-[#3E3A39] line-clamp-2 leading-snug flex-1">
          {item.itemName}
        </p>
